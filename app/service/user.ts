  ...
  */
    async userInfo() {
        ...
        if (ctx.session.currentRequestData) {
            id = ctx.session.currentRequestData.userInfo.id
        }
        if (id) {
            const res = await ctx.model.Users.findOne({
                where: { id },
     ...
     * 头像地址存在对应用户数据库
     */
    async updateAvatars(username: string, filename: string) {
        const { ctx } = this;
        let res = await ctx.model.Users.update({ avatar: `/public/avatars/${username}` + filename }, {
            where: {
                username,
            }
        })
    ...
    */
     ...
     * 注册
     */
    async register(payload: { username: string, password: string, email: string, code: string }) {
        const { ctx } = this;
        const { email_code } = ctx.session;
        console.log(email_code, 111)
        if (email_code && email_code.email === payload.email && email_code.code.toLowerCase() === payload.code.toLowerCase()) {
            // 验证注册是否重复
            //是否重名
            const isDuplicateName = await ctx.service.tools.uerTool.DuplicateUser(payload.username);
            if (!isDuplicateName) {
                const isDuplicateEmail = await ctx.service.tools.uerTool.DuplicateEmail(payload.email);
                if (!isDuplicateEmail) {
                    //进行写入数据库操作
                    return await this.registerDeal(ctx, payload);
                } else {
                    return {
                        //邮箱重复
                        __code_wrong: 40002,
     ...
     * 注册写入细节
     */
    async registerDeal(ctx, payload: { username: string, password: string, email: string, code: string }) {
        const { app } = ctx;
        const { username, password, email } = payload;
        const res = await app.model.Users.create({
            username,
            password,
            email,
        })
